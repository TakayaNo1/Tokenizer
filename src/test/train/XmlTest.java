...
//		XMLTokenizer xml=new XMLTokenizer(new File("D:/Download/国土交通省データ/鉄道データ/N02-08/KS-META-N02-08.xml"));
		...
		List<Node<HTMLElement>> curve_list=HTMLElement.search(root, dir+curve);
		...
		List<Node<HTMLElement>> point_list=HTMLElement.search(root, dir+point);
		...
		List<Node<HTMLElement>> eb02_list=HTMLElement.search(root, dir+eb02);//Reilroad Section
		int m=0;
		for(Node<HTMLElement> a:eb02_list){
			Node<HTMLElement> a_loc=HTMLElement.search(a, "ksj:LOC").get(0);
			String a_loc_id=a_loc.getElement().getMeta().get("idref");
			String a_loc_rac=HTMLElement.search(a, "ksj:RAC").get(0).getChildren(0).getElement().getContent();
		...
		List<Node<HTMLElement>> eb03_list=HTMLElement.search(root, dir+eb03);//Station
		...
		for(int i=0;i<10;i++){
			...
			Node<HTMLElement> a_loc=HTMLElement.search(a, "ksj:LOC").get(0);
			String a_loc_id=a_loc.getElement().getMeta().get("idref");
			String a_loc_name=HTMLElement.search(a, "ksj:STN").get(0).getChildren(0).getElement().getContent();
			String a_loc_rac=HTMLElement.search(a, "ksj:RAC").get(0).getChildren(0).getElement().getContent();
			...
			List<Node<HTMLElement>> a_indirect_points=HTMLElement.search(a_curve, "jps:GM_Curve.segment/jps:GM_LineString/jps:GM_LineString.controlPoint/jps:GM_PointArray/GM_PointArray.column/jps:GM_Position.indirect/GM_PointRef.point");
			List<Node<HTMLElement>> a_direct_points=HTMLElement.search(a_curve, "jps:GM_Curve.segment/jps:GM_LineString/jps:GM_LineString.controlPoint/jps:GM_PointArray/GM_PointArray.column/jps:GM_Position.direct/DirectPosition.coordinate");
			...
			for(Node<HTMLElement> p:a_indirect_points){
				...
				String coordinate=HTMLElement.search(a_point, "jps:GM_Point.position/jps:DirectPosition/DirectPosition.coordinate").get(0).getChildren(0).getElement().getContent();
